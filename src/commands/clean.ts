...
export const clean = async () => {
  // Support: dk clean nm or dk clean node_modules to delete node_modules only
  ...
  try {
    ...
    if (
      (cleanMode === "all" || cleanMode === "rn-android") &&
      existsSync("android/gradlew.bat")
    ) {
      cleanSpinner.text = "Running gradlew.bat clean..."
      await new Promise((resolve, reject) => {
        const gradle = spawn("cmd", ["/c", "cd android && gradlew.bat clean"], {
          stdio: "ignore",
        })
    ...
    ui.table([
      ...
      {
        key: "Files removed",
        value: itemsToClean.filter((i) => i.type === "files").length.toString(),
      },
